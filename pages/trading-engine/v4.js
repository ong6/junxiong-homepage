...
export default function TradingEngineV4() {
	return (
		<TradingVersionPage
			...
			lead="v4 took the model out and spent a month asking whether any of the strategies were real. Most of the work was audits, a stricter walk-forward, and monitors that can only say continue or kill."
			diagram={V4}
			...
			<P>
				...
				The sweep farm tried 51 candidates with no model calls at all. Live results were no help:
				not one book had reached a t-statistic of 2, so none was tuned on them. The harder finding
				was the ceiling. Of 30 swept candidates, 26 were indistinguishable from their benchmark and
				none was better, and the test only had even odds of detecting an edge near seven points a
				year. Small edges were invisible to it, and survivorship bias made that a limit of the data
				rather than of compute.
			</P>

			<H2>The audit of 2 September</H2>
			<P>
				A full read of the simulator found seven bugs. No dividend had ever been credited, which a
				backfill fixed with 26 credits worth $230.42. Monthly books would have skipped a month that
				ended on a weekend. There were $0 fills, a look-ahead rollback, and risk gates that trusted
				a typed price. The split checker had also restated three real crashes, BH, ORCL and NEM,
				as splits, and a later re-audit reverted 18 more. The engine had 228 tests after that audit
				and over 2,200 two weeks later.
			</P>

			<H2>Monitors that cannot promote</H2>
			<P>
				Fill model v4 added named execution profiles, so each result could be rerun at double cost
				or under a participation limit. Doubling costs at $39,000 barely moved SPY but took 1.85
				...
				Mondays and ends in May 2027. They report; they never act.
			</P>

			<P>
				By mid-September the engine was honest and very large. My own review on 18 September gave
				it 6 out of 10, with research output and scope control both at 3, so v5 started with an
				operating contract before it let an agent back in.
			</P>
		</TradingVersionPage>
	);
}
