...
export const skillforge = {
	...
	author: { label: "skillpack", sub: "SKILL.md · one folder per skill", icon: "doc" },
	evaluate: { label: "skillforge", sub: "frozen evaluation · scored", icon: "chart", baseline: "no-skill run" },
...
export const fieldpack = {
	figure: {
		...
		alt: "A laptop, an agent and a customer on the left use the fieldpack platform in the middle, whose cells are deckforge, skillforge, proofpack, the integration script, verification and the submodule pins; on the right sit the deck library, headless Chromium and the HTML handover.",
	},
	clients: [
		{ label: "Laptop", sub: "local-first", icon: "terminal" },
		{ label: "Agent", sub: "CLI · MCP", icon: "agent" },
		{ label: "Customer", sub: "reads the handover", icon: "user" },
	],
	platform: {
		title: "fieldpack",
		cells: [
			...
			{ label: "verification", sub: "pipeline · contract · browser" },
			{ label: "submodules", sub: "three repos, pinned" },
		],
	},
	resources: [
		{ label: "Deck library", sub: "versioned, local", icon: "blob" },
		{ label: "Headless Chromium", sub: "measured preflight", icon: "browser" },
...
export const deploy = (e2eCount) => ({
	...
	stages: [
		{ label: "Edit", sub: "pages · diagrams", icon: "doc" },
		{ label: "unslop", sub: "AI tells out", icon: "tool" },
		{ label: "Build", sub: "eslint · next build", icon: "terminal" },
		{ label: "Vercel", sub: "auto on push to main", icon: "cloud" },
...
export const connectorRule = {
	...
	before: {
		...
		stages: [
			...
			{ label: "Stub", sub: "a head at both ends", icon: "warning" },
			{ label: "Packet", sub: "sits on the head", icon: "warning" },
	...
	after: {
		...
		stages: [
			{ label: "Client", sub: "sends", icon: "client" },
			{ label: "Connector", sub: "one head, request direction", icon: "tool" },
			{ label: "Packet", sub: "reversed for the reply", icon: "tool" },
			{ label: "Service", sub: "replies", icon: "service" },
		],
		changed: [1, 2],
	},
